...
PnoP_phylum <- ggplot(data=woP, aes(x=time, y=tpm_mean, colour=Phylum, group=Phylum)) +
  ...
  xlab("Time (weeks)") + ylab("Cummulative abundance (TPM)") + 
  geom_errorbar(aes(ymin=tpm_mean-sd, ymax=tpm_mean+sd), width=0.1) + ylim(0, 800000)
...
# dsDNA: Duplorna unaffected, Pisu affected by P and time (7%, 7%). ssRNA+: Lenar by P and time (7%, 17%), Kitrino unaffected. ssRNA-: Negarna by P (8%).
...
PP <- ggplot(data=wP, aes(x=time, y=tpm_mean, colour=Phylum, group=Phylum)) +
  geom_line() + scale_colour_brewer(palette = "Dark2") +
  theme_bw() + ggtitle("P amended") +
  xlab("Time (weeks)") + ylab("Cummulative TPM") +  ylim(c(0, 400000)) +
  geom_errorbar(aes(ymin=tpm_mean-sd, ymax=tpm_mean+sd), width=0.1)
...
sig_taxa_down <- sig_taxa %>% filter(log2FoldChange < 0) # 1 Ourli
sig_taxa_up <- sig_taxa %>% filter(log2FoldChange > 0) # 28 mostly Wolfram and Levi
...
ann_colors = list(
    ...
    Order = c(Wolframvirales = "#FFFF99", Levivirales = "#6A3D9A", Tolivirales = "#E31A1C", Picornavirales = "#A6CEE3", Cryppavirales = "#FDBF6F", Tymovirales = "#33A02C"))
    #Order = c(Timlovirales = "#FFFF99", Levivirales = "#6A3D9A", Lenarviricota = "#CAB2D6", Tolivirales = "#B2DF8A", Ourlivirales = "#FF7F00", Picornavirales = "cyan", Cryppavirales = "#FDBF6F", Wolframvirales = "#B15928", Tymovirales = "darkgreen"))
